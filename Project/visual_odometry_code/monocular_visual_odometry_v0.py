...
def do_visual_odometry(monocular_visual_odometry, path_images, num_images, sequence_start_point, sequence_scale_point, seq):
    ...
    for img_id in range(num_images):
        ...
        if img_id > 0:
            img_i_1_th = cv.imread(path_images + str(img_id-1).zfill(6) + '.png', cv.IMREAD_GRAYSCALE)
        ...
        flag = monocular_visual_odometry.update_rotation_translation_matrix_of_camera(img_i_th, img_id)
        if flag is not None and flag == -1:
            monocular_visual_odometry.update_rotation_translation_matrix_of_camera_2(img_i_1_th, img_i_th, img_id)
...
class MonocularVisualOdometry:
    def __init__(self, cam, ground_truth):
        ...
        self.feature_detector_2 = cv.xfeatures2d.SURF_create()
    ...
    def update_rotation_translation_matrix_of_camera(self, img, frame_id):
        ...
        # if image is not first or second frame of sequence
            ...
            ref_keypoints_copy = np.copy(self.ref_keypoints)
            tracked_keypoints_copy = np.copy(self.tracked_keypoints)
            self.ref_keypoints, self.tracked_keypoints = key_points_tracking(prevImg=self.prev_frame, nextImg=self.new_frame,
                                                                             prevPts=self.ref_keypoints)
            ...
            if np.sum(mask_inliers) < 1700:
                self.ref_keypoints = ref_keypoints_copy
                self.tracked_keypoints = tracked_keypoints_copy
                return -1
            ...
            if self.ref_keypoints.shape[0] < 700:
    ...
    def update_rotation_translation_matrix_of_camera_2(self, img_pre, img, frame_id):
        """
        By receiving a i'th image, track features of (i-1)'th image in
        image i'th. then calculate Essential Matrix.
        By using essential matrix obtain position of camera.
        ...
        if frame_id == 0:
            ...
            # calculate key-points
            self.ref_keypoints_key = self.feature_detector_2.detect(self.new_frame)

            # calculate descriptors
            kp, des = self.feature_detector_2.compute(img, self.ref_keypoints_key)
        ...
        # if image is second frame of sequence
            ...
            # calculate key-points
            self.tracked_keypoints_key = self.feature_detector_2.detect(self.new_frame)

            # calculate descriptors
            kp, des = self.feature_detector_2.compute(img, self.tracked_keypoints_key)
        ...
        # if image is not first or second frame of sequence
            ...
            # coordinates of key-points
            self.ref_keypoints_coor = np.array([x.pt for x in self.tracked_keypoints_key], dtype=np.float32)
            tracked_keypoints_coor_copy = np.copy(self.ref_keypoints_coor)
            #############################
