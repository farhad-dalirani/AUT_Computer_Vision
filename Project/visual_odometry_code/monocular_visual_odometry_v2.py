...
class MonocularVisualOdometry:
    def __init__(self, cam, ground_truth):
        ...
        # feature detector
        self.feature_detector = cv.ORB_create(nfeatures=3500, nlevels=16)
    ...
    def update_rotation_translation_matrix_of_camera(self, img, frame_id):
        ...
        # if image is not first or second frame of sequence
            ...
            print('number of ones', np.sum(np.where(mask_inliers==255,1,0)))
            print('number of zeros', np.sum(np.where(mask_inliers == 255, 0, 1)))
